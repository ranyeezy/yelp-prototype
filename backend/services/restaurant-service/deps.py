...
oauth2_user_or_owner = OAuth2PasswordBearer(tokenUrl="/auth/users/login", description="Accepts tokens from either /auth/users/login or /auth/owners/login")
...
def get_current_user_or_owner(token: str = Depends(oauth2_user_or_owner), db: Session = Depends(get_db)):
    payload = _decode_token(token)
    token_role = payload.get("role")
    user_id = payload.get("sub")
    ...
    if token_role == "user":
        ...
        if not user:
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="User not found")
        return user
    elif token_role == "owner":
        owner = db.get(models.Owner, int(user_id))
        if not owner:
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Owner not found")
        return owner
    else:
