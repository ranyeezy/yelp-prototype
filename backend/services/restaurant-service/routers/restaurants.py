...
def _assert_can_modify(db: Session, current_user, restaurant):
    if isinstance(current_user, models.Owner):
        claim = (
            db.query(models.OwnerRestaurant)
            .filter_by(owner_id=current_user.id, restaurant_id=restaurant.id)
            .first()
        )
        if not claim:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="You have not claimed this restaurant")
    elif restaurant.listed_by_user_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="You can only modify your own restaurant listings")
...
def update_restaurant(
    ...
    _assert_can_modify(db, current_user, restaurant)
